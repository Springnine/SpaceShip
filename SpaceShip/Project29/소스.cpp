...
void RunGame()
{
	...
	{
		if (sy < 400 - 12 - 2.3) sy += 2.3;
	}

	//우주선의 영역. 2만큼 여유를 둔다.
	SetRect(&srt, (int)sx + 3, (int)sy + 3, (int)sx + 12, (int)sy + 12);
	...
	wsprintf(sEllapse, TEXT("%d.%.2d초"), (GetTickCount() - st) / 1000, ((GetTickCount() - st) % 1000)/10);
